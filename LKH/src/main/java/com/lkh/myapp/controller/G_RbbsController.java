...
@Slf4j
@RestController
@RequestMapping("/grbbs")
public class G_RbbsController {

	private static Logger logger = LoggerFactory.getLogger(G_RbbsController.class);
	...
	public ResponseEntity<String> write(@RequestBody G_RbbsDTO grdto) {
	...
	public ResponseEntity<String> modify(@RequestBody G_RbbsDTO grdto) {
		ResponseEntity<String> resCode = null;
		if (grdto == null) {
			resCode = new ResponseEntity<String>("fail", HttpStatus.BAD_REQUEST);
			return resCode;
	...
	public ResponseEntity<String> reply(@RequestBody G_RbbsDTO grdto) {
		ResponseEntity<String> resCode = null;
		if (grdto == null) {
			resCode = new ResponseEntity<String>("fail", HttpStatus.BAD_REQUEST);
			return resCode;
	...
	public ResponseEntity<Map<String,Object>> map(
		...
		RecordCriteria rc = new RecordCriteria(reReqPage, 20);

		try {
			// 페이지 처리
			// =-----------------------------------------------------------------------
			PageCriteria pc = new PageCriteria(rc, grbbsSvc.replyTotalRec(num), 20);
			// -------------------------------------------------------------------------------

			map.put("item", grbbsSvc.list(num, rc.getStartRecord(), rc.getEndRecord()));
