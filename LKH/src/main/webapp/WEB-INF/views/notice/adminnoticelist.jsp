...
<body>
	...
	<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
		...
		<div class="btn-toolbar mb-2 mb-md-0">
		        ...
		        <form  action="/notice/adminnotice" method="post">
		       	<div class="row">
	...
	<div class="table-responsive">
					...
					<tr>
						...
						<td>${noticeDTO.num }</td>
						 <td>
							...
							<a class="text-reset" href="/notice/view?num=${noticeDTO.num }&${pc.makeSearchURL(pc.recordCriteria.reqPage)} " class="text-decoration-none text-reset text-danger" >${noticeDTO.title }</a>
						</td>
		...
		<div>
			<a class="btn btn-sm btn-outline-secondary" href="/notice/write">공지사항 작성</a>
		</div>
	</div>
	...
	<ul class="pagination pagination-sm justify-content-center">
		...
		<c:if test="${pc.prev }">
			<li class="page-item"><a class="page-link"
				href="/notice/adminnotice?${pc.makeSearchURL(1) }&searchType=${pc.searchType}&keyword=${pc.keyword}">처음</a></li>
			<li class="page-item"><a class="page-link"
				href="/notice/adminnotice?${pc.makeSearchURL(pc.startPage - 1) }&searchType=${pc.searchType}&keyword=${pc.keyword}">이전</a></li>
		</c:if>
		...
		<c:forEach begin="${pc.startPage }" end="${pc.endPage }" var="pageNum">
			...
			<c:if test="${pc.recordCriteria.reqPage != pageNum }">
				<li class="page-item"><a class="page-link"
					href="/notice/adminnotice?${pc.makeSearchURL(pageNum) }&searchType=${pc.searchType}&keyword=${pc.keyword}">${pageNum }</a>
				</li>
			</c:if>
		...
		<c:if test="${pc.next }">
			<li class="page-item"><a class="page-link"
				href="/notice/adminnotice?${pc.makeSearchURL(pc.endPage + 1) }&searchType=${pc.searchType}&keyword=${pc.keyword}">다음</a></li>
			<li class="page-item"><a class="page-link"
				href="/notice/adminnotice?${pc.makeSearchURL(pc.finalEndPage) }&searchType=${pc.searchType}&keyword=${pc.keyword}">마지막</a></li>
		</c:if>
	</ul>
