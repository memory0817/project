...
<body>
	...
	<div
		...
		<h1 class="h2">공지사항_답글</h1>
	</div>

	<form:form modelAttribute="noticeDTO" action="/notice/replyOK" method="post">
		<form:hidden  path="bgroup" value="${noticeDTO.bgroup }"/>
			<form:hidden  path="step" value="${noticeDTO.step }"/>
			<form:hidden  path="indent" value="${noticeDTO.indent }" />
		...
		<table class="table table-sm" summary="게시글 보기">
			...
			<tbody>
				...
				<tr>
					<th>작성자</th>
					<td><form:input class="form-control is-valid" path="name"
							type="text"  value="${user.id }"/> </td>
				</tr>
				<tr>
					<th>내용</th>
					<td>
							<form:textarea class="form-control is-valid"
							placeholder="내용을 입력하세요." path="content" rows="15" cols="30" />
