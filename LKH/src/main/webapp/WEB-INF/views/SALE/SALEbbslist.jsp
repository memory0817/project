...
<script>
 $(function(){
	 $("#write").on("click",function(e){
		 e.preventDefault();
		 location.href="${pageContext.request.contextPath }/SALE/write"; 			 	 
	 });	
...
<body>
	...
	<div class="table-responsive">
		<table class="table table-striped table-sm">
			...
			<tbody>
				<c:forEach items="${list }" var="salebbsdto">
				<c:if test="${salebbsdto.isdel eq 'Y'}">
					<tr>
						<td>${salebbsdto.num}</td>
				...
				<c:if test="${salebbsdto.isdel ne 'Y'}">
					<tr>
						<td>${salebbsdto.num }</td>
						 <td>
							<c:forEach begin="1" end="${salebbsdto.indent }">&nbsp;&nbsp;</c:forEach>
	            <c:if test="${salebbsdto.indent > 0 }">
	            	<img src="${pageContext.request.contextPath }/resources/images/re.png" width="20" style="dispaly=inline" >
	            </c:if>				
						<a href="/SALE/view?num=${salebbsdto.num }&${pc.makeSearchURL(pc.recordCriteria.reqPage)} " class="text-decoration-none text-reset text-danger" >${salebbsdto.title }</a>
						</td>
						<td>${salebbsdto.name }</td>
						<td>${salebbsdto.position }</td>
						<td>${salebbsdto.cdate }</td>
						<td>${salebbsdto.hit }</td>
					</tr>
				</c:if>
				</c:forEach>
	...
	<ul class="pagination pagination-sm justify-content-center">
		...
		<c:if test="${pc.prev }">
			<li class="page-item"><a class="page-link"
				href="/SALE/list?${pc.makeSearchURL(1) }&searchType=${pc.searchType}&keyword=${pc.keyword}">처음</a></li>
			<li class="page-item"><a class="page-link"
				href="/SALE/list?${pc.makeSearchURL(pc.startPage - 1) }&searchType=${pc.searchType}&keyword=${pc.keyword}">이전</a></li>
		</c:if>
		...
		<c:forEach begin="${pc.startPage }" end="${pc.endPage }" var="pageNum">
			...
			<c:if test="${pc.recordCriteria.reqPage != pageNum }">
				<li class="page-item"><a class="page-link"
					href="/SALE/list${pc.makeSearchURL(pageNum) }&searchType=${pc.searchType}&keyword=${pc.keyword}">${pageNum }</a>
				</li>
			</c:if>
		...
		<c:if test="${pc.next }">
			<li class="page-item"><a class="page-link"
				href="/SALE/list${pc.makeSearchURL(pc.endPage + 1) }&searchType=${pc.searchType}&keyword=${pc.keyword}">다음</a></li>
			<li class="page-item"><a class="page-link"
				href="/SALE/list${pc.makeSearchURL(pc.finalEndPage) }&searchType=${pc.searchType}&keyword=${pc.keyword}">마지막</a></li>
		</c:if>
	</ul>
