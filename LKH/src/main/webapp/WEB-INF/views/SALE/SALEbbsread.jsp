...
<script>
	$(function () {
		...
		$("#btn1").on("click", function (e) {
			location.href = "/SALE/replyForm/${sale_bbsDTO.num}/${rc.reqPage}";
		});
		
		//수정버튼 클릭시 
		$("#btn2").on("click", function (e) {
			location.href = "/SALE/modifyform/${sale_bbsDTO.num}/${rc.reqPage}";
			
		});

		//삭제버튼
		$("#btn3").on("click", function (e) {
			location.href = "/SALE/delete?num=${sale_bbsDTO.num}&reqPage=${rc.reqPage}";
		});
...
<c:set var='user' value="${SPRING_SECURITY_CONTEXT.authentication.principal }"/>
	...
	<main role="main" class="col-md-9 col-lg-10" style="padding-left:20%;"> 
		<div
		class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
		<h1 class="h2">게시글읽기</h1>
		</div>
		<div class="table-responsive">
			<form:form modelAttribute="sale_bbsDTO" action=""	method="post">
				<form:hidden path="num" value="${sale_bbsDTO.num }" />
					...
					<tbody>
						<tr>
							...
							<td><form:input class="form-control is-valid" path="title"
									type="text" placeholder="제목을 입력하세요" readonly="true"	value="${sale_bbsDTO.title }" /> 
									<form:errors class="valid-feedback" path="title"></form:errors></td>
						</tr>
						<tr>
							<th>작성자</th>
							<td>&nbsp;${sale_bbsDTO.name } </td>
						</tr>
						<tr>
							<th>작성자</th>
							<td>&nbsp;${sale_bbsDTO.position } </td>
						</tr>
						<tr>
							<th>내용</th>
							<td>
								<form:textarea class="form-control is-valid" path="content" rows="15" cols="30"	
								value="${sale_bbsDTO.content }" readonly="true" /> 
						...
						<tr>
							<td colspan="2" align="right">
									...
									<c:if test="${sale_bbsDTO.name eq user.name or admin}">						
				...
				<jsp:include page="../reply/SALE_reReply.jsp" flush="false" />
			</form:form>
		</div>
	</main>
</body>
