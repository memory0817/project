...
<script>
		...
		var num = ${test_bbsDTO.num};		// 원글 번호  
		var rereqPage = 1;	// 요청페이지
		
 		
 		var $login_id = "${user.id}"; // 로그인 아이디
...
/* 			 $('#rcontent').val(function(){
	...
	function replyList(rereqPage){
		...
		$.ajax({
			type: "GET",
			url: "/testrbbs/posts/map/"+num+"/"+rereqPage,
					dataType:"JSON",
					success: function(data,status,xhr){
						...
						str += "<ul class='list-gruop list-group-flush'>";
 						...
 						$.each(data.item,function(idx,rec){
							...
							if(rec.test_rdto != null){
								rrid = '@'+rec.test_rdto.rid; //부모댓글의 아이디
							}
							...
							if(rec.isdel == 'Y'){
 							...
 							// 들여쓰기
 								...
 								str += "</div>";
 								str += "<div class='col-11'>";
 								str += "<h6>"+rec.rid+rec.loginid+" <small><i>"+rec.rcdate+"</i></small></h6>";
...
<div>
  ...
  <div class='toast-header'>
    <img src='/resources/images/basicicon.jsp' style='width:20px;' class='rounded mr-2' alt='알림메세지'>
