...
<script>
	$(function () {
		...
		$("#btn1").on("click", function (e) {
			location.href = "/IT/replyForm/${it_bbsDTO.num}/${rc.reqPage}";
		});
		
		//수정버튼 클릭시 
		$("#btn2").on("click", function (e) {
			location.href = "/IT/modifyform/${it_bbsDTO.num}/${rc.reqPage}";
			
		});

		//삭제버튼
		$("#btn3").on("click", function (e) {
			location.href = "/IT/delete?num=${it_bbsDTO.num}&reqPage=${rc.reqPage}";
		});
...
<c:set var='user' value="${SPRING_SECURITY_CONTEXT.authentication.principal }"/>
	...
	<main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
		...
		<div class="table-responsive">
			<form:form modelAttribute="it_bbsDTO" action=""	method="post">
					...
					<tbody>
						<tr>
							...
							<td><form:input class="form-control is-valid" path="title"
									type="text" placeholder="제목을 입력하세요" readonly="true"	value="${it_bbsDTO.title }" /> 
									<form:errors class="valid-feedback" path="title"></form:errors></td>
						</tr>
						<tr>
							<th>작성자</th>
							<td>&nbsp;${it_bbsDTO.name } </td>
						</tr>
						<tr>
							<th>작성자</th>
							<td>&nbsp;${it_bbsDTO.position } </td>
						</tr>
						<tr>
							<th>내용</th>
							<td>
								<form:textarea class="form-control is-valid" path="content" rows="15" cols="30"	
								value="${it_bbsDTO.content }" readonly="true" /> 
						...
						<tr>
							<td colspan="2" align="right">
									...
									<c:if test="${it_bbsDTO.name eq user.name or admin}">								
				...
				<jsp:include page="../reply/IT_reReply.jsp" flush="false" />
			</form:form>
		</div>
	</main>
</body>
