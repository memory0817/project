...
<script>
 $(function(){
	 $("#write").on("click",function(e){
		 e.preventDefault();
		 location.href="${pageContext.request.contextPath }/IT/write"; 			 	 
	 });	
...
<body>
	...
	<div class="table-responsive">
		<table class="table table-striped table-sm">
			...
			<tbody>
				<c:forEach items="${list }" var="itbbsdto">
				<c:if test="${itbbsdto.isdel eq 'Y'}">
					<tr>
						<td>${itbbsdto.num}</td>
				...
				<c:if test="${itbbsdto.isdel ne 'Y'}">
					<tr>
						<td>${itbbsdto.num }</td>
						 <td>
							<c:forEach begin="1" end="${itbbsdto.indent }">&nbsp;&nbsp;</c:forEach>
	            <c:if test="${itbbsdto.indent > 0 }">
	            	<img src="${pageContext.request.contextPath }/resources/images/re.png" width="20" style="dispaly=inline" >
	            </c:if>				
						<a href="/IT/view?num=${itbbsdto.num }&${pc.makeSearchURL(pc.recordCriteria.reqPage)} " class="text-decoration-none text-reset text-danger" >${itbbsdto.title }</a>
						</td>
						<td>${itbbsdto.name }</td>
						<td>${itbbsdto.position }</td>
						<td>${itbbsdto.cdate }</td>
						<td>${itbbsdto.hit }</td>
					</tr>
				</c:if>
				</c:forEach>
	...
	<ul class="pagination pagination-sm justify-content-center">
		...
		<c:if test="${pc.prev }">
			<li class="page-item"><a class="page-link"
				href="/IT/list?${pc.makeSearchURL(1) }&searchType=${pc.searchType}&keyword=${pc.keyword}">처음</a></li>
			<li class="page-item"><a class="page-link"
				href="/IT/list?${pc.makeSearchURL(pc.startPage - 1) }&searchType=${pc.searchType}&keyword=${pc.keyword}">이전</a></li>
		</c:if>
		...
		<c:forEach begin="${pc.startPage }" end="${pc.endPage }" var="pageNum">
			...
			<c:if test="${pc.recordCriteria.reqPage != pageNum }">
				<li class="page-item"><a class="page-link"
					href="/IT/list${pc.makeSearchURL(pageNum) }&searchType=${pc.searchType}&keyword=${pc.keyword}">${pageNum }</a>
				</li>
			</c:if>
		...
		<c:if test="${pc.next }">
			<li class="page-item"><a class="page-link"
				href="/IT/list${pc.makeSearchURL(pc.endPage + 1) }&searchType=${pc.searchType}&keyword=${pc.keyword}">다음</a></li>
			<li class="page-item"><a class="page-link"
				href="/IT/list${pc.makeSearchURL(pc.finalEndPage) }&searchType=${pc.searchType}&keyword=${pc.keyword}">마지막</a></li>
		</c:if>
	</ul>
